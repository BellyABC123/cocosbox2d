...
PlayerPaddle::PlayerPaddle():power(0), m_fInitPositionY(0) {
...
PlayerPaddle* PlayerPaddle::paddleWithTexture(CCTexture2D* aTexture, b2World* world,const CCPoint& position,const CCRect& rect)
{
	...
	pPaddle->setInitPositionY(position.y);
    return pPaddle;
}
...
bool PlayerPaddle::ccTouchBegan(CCTouch* touch, CCEvent* event)
{
	...
	if(!rect.containsPoint(location))
		return false;
			...
			b2Vec2 b2location = b2Vec2(location.x/PTM_RATIO, m_fInitPositionY/PTM_RATIO);
